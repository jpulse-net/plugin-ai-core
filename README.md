...
The panel accepts site `regions` at named anchors and a complete `commands` list (`jPulse.ai.commands.defaults` is the spread). `/help` command names and examples may be clickable `[[label]]` rows; text after the brackets is a note. `adapter.contextOptions()` shows a context row; `handle.context` keeps it in sync with the page. Sites that implemented `describeScope` on the adapter can delete it — scope labels live on `onAiScopeResolve`. Chips are `handle.attachments()` (`kind: 'source' | 'image'`); `handle.attachmentFile(id)` returns the original file. A chip ⋯ **Attach** appears only when `adapter.attach` exists; `canAttach` may disable it. That write is not a proposal. A deleted object is `AiCore.deleteByScope({ scopeType, scopeId })`. `create({ title })` sets the toolbar label; `handle.setTitle` restamps it. `storageKey`, `cascade`, `group`, `mobile`, `defaults`, `minWidth`, and `minHeight` are forwarded to the float panel (`group` is `mobile.exclusive` only). Double-click the toolbar to restore the default size and corner (`resetOnTitleDblclick: false` opts out). `create()` returns `destroy()`, which cancels a running turn, unregisters the float panel, closes the per-thread WebSocket, and removes the body node. Cancel (button or `/cancel`) returns the compose row to Send. The conversation picker omits an archived conversation whose turns have aged out. A trash button beside rename and (+) deletes the conversation currently open (not a row inside the picker). `/delete` does the same; both confirm with `jPulse.UI.confirmDialog` and name the title. Empty conversations and a running turn are no-ops. A second tab discovers a deleted conversation on its next list or open. A conversation whose oldest surviving turn is past seq 1 shows “Older messages were removed after N days.” The “kept for N days” policy is a one-shot info toast when the user opens or uses the panel, not on page load and not a standing banner. `/new` and a conversation switch confirm when chips are attached (switch uses its own title and primary). Compose Enter does not bubble. Compose paste of text stays in the box; only clipboard files and images become chips. Send queues until the socket is open.
