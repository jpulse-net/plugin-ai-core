...
describe('slash catalog', () => {
    ...
    test('slash picker matches the typed prefix and ignores literals', () => {
        ...
        expect(filterSlashCommands('/', catalog, {}).map((row) => row.name)).toEqual([
            'help', 'tools', 'model', 'new', 'cancel', 'delete', 'conversations', 'status'
        ]);
...
describe('adapter contract scan', () => {
    ...
    test('hello-ai has no propose_image; that contract lives on the panel handle', () => {
        ...
        const helloView = fs.readFileSync(
            path.resolve(process.cwd(), 'plugins/hello-ai/webapp/view/hello-ai/index.shtml'),
            'utf8'
        );
