# jPulse Docs / Installed Plugins / AI Core Plugin v1.0.15
...
Prompt and response debug dumps need both the plugin `debugDumps` setting and the host `aiCore` debug area on Admin → Logs. Requires jPulse Framework >= 2.0.8.
...
`adapter` may be omitted. `toolData` plus `describeContext` / `describeTarget` are enough for a read-only agent. Scope labels belong on `onAiScopeResolve`, not the adapter. `executeTool` is the exception, not the interface. The three `*Proposal` methods are used only when a registered tool declares `proposes: true`. `canUndoProposal(proposal)` is optional: return false to hide Undo (hello-ai does this when the tab snapshot is gone). Omit it and Undo stays after a successful apply. `contextOptions()` is optional: implement it and the panel shows a context row and `/context`; omit it and neither appears.
...
| `/delete` | Delete the open conversation after a confirm. No `/delete n` — open one with `/conversations n`, then delete |
...
The model is not in the panel header. `/model` is how you view and set it. Conversation title, switch, rename, new, and delete sit on one row under the title.
